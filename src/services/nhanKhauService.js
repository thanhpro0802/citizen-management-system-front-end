...
async searchNhanKhau(criteria = {}, page = 0, size = 10) {
    const filteredCriteria = Object.entries(criteria).reduce(
    (acc, [key, value]) => {
...
async getNhanKhauById(maNhanKhau) {
    const response = await fetchWithAuth(
    `${API_BASE_URL}/nhan-khau/${maNhanKhau}`,
    {
        method: 'GET',
    }
    );
...
async registerTamTru(data) {
    const response = await fetchWithAuth(
    `${API_BASE_URL}/nhan-khau/${data.maNhanKhau}/tam-tru`,
    {
        method: 'POST',
        body: JSON.stringify(data),
    }
...
async registerTamVang(data) {
    const response = await fetchWithAuth(
    `${API_BASE_URL}/nhan-khau/${data.maNhanKhau}/tam-vang`,
    {
        method: 'POST',
        body: JSON.stringify(data),
    }
...
async declareDeath(maNhanKhau) {
    const response = await fetchWithAuth(
    `${API_BASE_URL}/nhan-khau/${maNhanKhau}/khai-tu`,
    {
        method: 'POST',
    }
    );
...
async layThongTinNhanKhauCuaToi() {
    const response = await fetchWithAuth(
    `${API_BASE_URL}/nhan-khau/cua-toi`,
    {
        method: 'GET',
    }
    );
